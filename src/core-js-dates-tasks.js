...
function getNextFriday(date) {
  ...
  return new Date(
    d.setHours(d.getHours() + ((7 - dayOfWeek + 5) % 7 || 7) * 24)
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const msDateDiff = Date.parse(dateEnd) - Date.parse(dateStart);

  return msDateDiff / msPerDay + 1;
}
...
function isDateInPeriod(date, period) {
  const msStartDate = Date.parse(period.start);
  const msEndDate = Date.parse(period.end);
  const msGivenDate = Date.parse(date);

  return msStartDate <= msGivenDate && msGivenDate <= msEndDate;
}
...
function getWeekNumberByDate(date) {
  let daysSum = date.getDate();

  for (let i = 0; i < date.getMonth(); i += 1) {
    daysSum += new Date(date.getFullYear(), i + 1, 0).getDate();
  }

  return Math.ceil(daysSum / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  let currentDate = new Date(year, month, 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const splittedStartDate = period.start.split('-').map(Number);
  let startDay = splittedStartDate[0];
  const [, startMonth, startYear] = splittedStartDate;
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      currentDate = new Date(startYear, startMonth, startDay);
    }

    startDay += countOffDays;
    currentDate = new Date(startYear, startMonth, startDay);
  }
